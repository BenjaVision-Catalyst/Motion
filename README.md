...
Motion also supports behaviours whose values come from somewhere other than interpolation. [Catalyst Physics](https://github.com/BenjaVision-Catalyst/Physics) uses this to present simulation results through the same machinery as ordinary motion. This means an element can combine animation, interaction and simulation while still arriving at Bloc through a single presentation path.
